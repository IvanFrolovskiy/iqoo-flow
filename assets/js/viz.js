...
const CTX = [
  ...
  () => card('Из 100 студентов вузов', 'VK Education и «Твой Ход», сентябрь 2026, 1 500 студентов из 8 федеральных округов',
    ...
    'В источнике написано «около 60%», сумма 61 получилась из-за округления.'),
  () => card('Игры и нейросети', 'РВИ, ИФСИ и НАФИ, октябрь 2025 · «Я — профессионал», февраль 2025',
    ...
    '7 из 10 игроков играют на телефоне или планшете.'),
  () => card('237 уведомлений за день', 'медиана, подростки 11–17 лет, США · Common Sense Media, 2023',
    ...
    'Один кружок — одно уведомление. Подростки открывают примерно каждое четвёртое.'),
];
...
const COST = [
  () => card('Сколько секунд внимание держится на одном экране', 'в среднем · наблюдения Глории Марк, UC Irvine',
    ...
    'За 20 лет время сократилось втрое.'),
  () => card('Час работы и два отвлечения', 'схема по оценке Г. Марк: на возвращение к задаче уходит около 25 минут',
    `<div class="tl"><div class="tl-bar">
      ...
      <div class="tl-lab"><span style="--w:20%">12 мин дела</span><span style="--w:41.6%">уведомление, потом ≈25 мин на возвращение</span><span style="--w:13.4%">8 мин</span><span style="--w:25%">снова…</span></div>
    ...
    'Схема условная: она показывает, из чего складывается цена двух отвлечений.'),
  () => card('Самоконтроль и сон', 'SuperJob, март 2026 · ORO для РИА Новости, май 2026',
    `<div class="tiles">
      ...
      <div class="tile"><div class="v">57%</div><div class="l">россиян даже не пытаются себя ограничивать</div><div class="meter" style="--p:.57"><b></b></div></div>
...
const INS = [
  () => card('Детокс и отключение уведомлений', 'ВЦИОМ, 2023 · Go Mobile, ноябрь 2025',
    ...
    'Отключая уведомления приложения, человек теряет и спам, и нужные сообщения.'),
  () => card('Блокировщики и кнопочные телефоны', 'Fortune, февраль 2026',
    `<div class="cards3">
      <div class="pcard"><div class="ico"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><rect x="4" y="6" width="16" height="12" rx="3"/><path d="M8 12h8"/></svg></div><div class="pv">$39</div><div class="pl">Bloom: металлическая карточка, после касания телефоном блокирует выбранные приложения. Продано больше 60&nbsp;000</div><div class="minus">отдельный предмет, его легко забыть дома</div></div>
      <div class="pcard"><div class="ico"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><rect x="5" y="5" width="14" height="14" rx="4"/><circle cx="12" cy="12" r="2.5"/></svg></div><div class="pv">$59</div><div class="pl">Brick: магнитный блок, работает так же, как Bloom</div><div class="minus">ещё одно устройство и приложение</div></div>
      <div class="pcard"><div class="ico"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><rect x="7" y="3" width="10" height="18" rx="2"/><path d="M10 15h4M10 18h4"/></svg></div><div class="pv">Кнопочный</div><div class="pl">Кнопочный телефон вместо смартфона: самый радикальный вариант</div><div class="minus">нет оплаты, навигатора, учёбы и игр</div></div></div>`),
  () => card('Позиционирование', 'экспертная оценка, не данные опросов',
    ...
    'Производительного телефона с удобным управлением вниманием на рынке пока нет.'),
];
  ...
  if (struct) {
    ...
    struct.innerHTML = `<p class="viz-title">Из чего складываются 59 990 ₽</p><p class="viz-sub">модельная оценка по открытым данным · курс 84 ₽/$</p>` +
      ...
      `<table class="vtable">${STRUCT.map(s => `<tr><td><i style="--c:${s.c}"></i>${s.label}</td><td>${fmt(s.v)} ₽</td></tr>`).join('')}</table>
       <p class="viz-note">Себестоимость ≈ $365. Допущения: наценка розницы 12% от цены без НДС, расходы бренда 17% и его маржа 12% от оптовой цены. Реальную структуру цены iQOO не публикует.</p>`;
  }
