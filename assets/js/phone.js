...
export function createPhone(host, { labels = true, interactive = false, onToggle } = {}) {
  host.innerHTML = `<div class="aura"></div>
    <div class="phone" data-mode="life">
      ...
      <div class="screen"><div class="punch"></div><div class="layer" data-l="a"></div><div class="layer out" data-l="b"></div></div>
    </div>`;
  const phone = host.querySelector('.phone');
  const layers = [...host.querySelectorAll('.layer')];
  let front = 0, mode = 'life';
  const api = {
    el: phone,
    ...
    show(html, { instant = false } = {}) {
      const next = layers[1 - front], cur = layers[front];
      next.innerHTML = html;
      if (instant) { cur.classList.add('out'); next.classList.remove('out'); }
      else requestAnimationFrame(() => { cur.classList.add('out'); next.classList.remove('out'); });
      front = 1 - front;
      return next;
    },
    update(html) { layers[front].innerHTML = html; return layers[front]; },
  };
  if (interactive) {
...
export const sb = (time, bat, low = false) =>
  `<div class="sb"><span>${time}</span><span class="bat${low ? ' low' : ''}">${bat}% <i style="--b:${bat}"></i></span></div>`;
...
export const nzbat = (f) => `<div class="nzbat"><div class="cap"><span>заряд ${f}%</span><span>НЗ 10% не тратится</span></div>
  ...
  <div class="legend"><span><b>■</b> резерв на оплату, карты, звонки и мессенджеры</span></div></div>`;
...
export const SCENES = [
  ...
  { mode: 'focus', html: () => `<div class="scr">${sb('18:41', 31)}${modebar('focus', 'до 22:00')}
      ...
      <div class="panel"><h5><i></i>Шумодав держит 146</h5>
        <div class="row"><span>Пропускаю</span><b>Лёша · научрук · мама · банк</b></div>
  ...
  { mode: 'focus', html: () => `<div class="scr">${sb('19:25', 27)}${modebar('focus', 'ещё 2:35')}
      ...
      ${ntf({ ic: 'Л', c: '#34d3c1', who: 'Лёша · проект', text: 'Скинул правки к главе 3 — глянь таблицу 2, сдаём сегодня', tm: '19:25', cls: 'hot',
        tags: tagline([['проект', 1], ['дедлайн сегодня', 1], ['пропущено Шумодавом', 0]]) })}
  ...
  { mode: 'focus', html: () => `<div class="scr">${sb('21:00', 21)}${modebar('focus', 'перерыв 10 мин')}<div class="tags" style="justify-content:center;margin-top:-1%"><span class="m">⚡ на зарядке · 100 Вт</span></div>
      <div class="panel" style="margin-top:4%"><h5><i></i>Сводка за 2 ч 20 мин</h5>
        <div class="row"><span>Пришло</span><b>96</b></div><div class="row"><span>Пропущено сразу</span><b>2</b></div><div class="row"><span>В сводке</span><b>94 → 3 строки</b></div></div>
      <div class="stack">
  ...
  { mode: 'play', html: () => `<div class="hud">${sb('22:35', 58)}
      <div class="modebar" style="margin-top:2%"><i></i>Игра · на полную</div>
